...
int main(int argc,char *argv[]){
  ...
  HashTable<pair<int,vector<pair<double,double>>>> *htable = new HashTable<pair<int,vector<pair<double,double>>>>[1];
  ...
  int d=read_data(true,bf,trueNN,avg,10,true,traversals,GV,5,input,query,1,n1,n2,inputPoints,queryPoints,inputVector,queryVector);
  ...
  findNN(max,true,trueNN,avg,false,1,n2,queryPoints,queryVector,w,d,k,M,mArray,gs,htable,dt,F,um,probes,output);
  for(int i=0;i<dt;i++){
    delete gs[i];
	}
  delete[] mArray;
